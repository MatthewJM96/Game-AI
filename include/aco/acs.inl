...
void aco::acs::prepare_ants(Ant<MapSize, MaxSteps>* ants, size_t ant_count, AntColony<MapSize, MaxSteps>* ant_colony) {
    for (size_t i = 0; i < ant_count; ++i) {
        ...
        ants[i].visited_nodes.fill(false);
    }
}

template <size_t MapX, size_t MapY, size_t MaxSteps>
void aco::acs::create_pheromone_heatmap_frame(std::string tag, AntColony<MapX * MapY, MaxSteps>& ant_colony) {
    static float saturation_point = 200.0f;
...
template <size_t MapX, size_t MapY, size_t MaxSteps>
void aco::acs::create_ant_count_heatmap_frame(std::string tag, AntColony<MapX * MapY, MaxSteps>& ant_colony) {
    heatmap_t* heatmap = heatmap_new(MapX * 10, MapY * 10);
    ...
    uint8_t* image_data = new uint8_t[MapX * 10 * MapY * 10 * 4];
    heatmap_render_saturated_to(heatmap, heatmap_cs_default, (float)ant_colony.ant_count, image_data);
...
size_t aco::acs::choose_next_node(Ant<dimension::dim2d_to_padded_size(MapDim), MaxSteps>* ant, float exploitation_factor, float(*to_node_cost)(VertexDescriptor initial, VertexDescriptor final), float cost_exponent) {
    size_t padded_dim = MapDim + 2;
    ...
    auto do_next_node_check = [&](EdgeDescriptor edge, VertexDescriptor initial_vertex, VertexDescriptor candidate_vertex) {
        ...
        if (ant->visited_nodes[actual_map->vertex_to_map_idx_map[candidate_vertex]]) return;
    ...
    while (iteration > 0) {
        ...
        for (size_t step = 0; step < 2 * MaxSteps; ++step) {
            ...
            if (step % options.output_frequency == 0) {
                ...
                create_pheromone_heatmap_frame<padded_dim, padded_dim, MaxSteps>("pngs/" + std::to_string(options.iterations - iteration + 1) + "." + std::to_string(step + 1) + ".acs.pheromone", ant_colony);
                create_ant_count_heatmap_frame<padded_dim, padded_dim, MaxSteps>("pngs/" + std::to_string(options.iterations - iteration + 1) + "." + std::to_string(step + 1) + ".acs.ant_count", ant_colony);
            }
            ...
            for (size_t ant_idx = 0; ant_idx < options.ant_count; ++ant_idx) {
                ...
                if (ant->has_food) {
                    ...
                    ant->visited_nodes[next_node_idx] = true;
                    ant->current_node_idx = next_node_idx;
                    ant->steps_taken += 1;
